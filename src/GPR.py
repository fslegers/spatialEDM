...
def GPR_forecasting(X_trains, y_trains, X_tests, y_tests, dim):
    ...
    for X_train, y_train, X_test, y_test in zip(X_trains, y_trains, X_tests, y_tests):
        ...
        pred_this_fold = gpr.predict(X_test, return_std=True)
        diff = np.abs(np.subtract(y_test, pred_this_fold))

        results['mae'] += mean(diff)
        results['rmse'] += math.sqrt(mean(np.square(diff)))

        try:
            results['corr'] += pearsonr(y_test, pred_this_fold)[0]
        except:
...
def GPR_training(ts, lag=1, dim=10, cv_method="LB"):

    # Embed time series
    lib = embed_time_series(ts, lag=lag, E=dim)

    # Split into training and test set
    if cv_method == "RB" or cv_method == "RBCV":
    ...
    results = GPR_forecasting(X_trains, y_trains, X_tests, y_tests, dim)
...
# TODO: moving-window cross validation
