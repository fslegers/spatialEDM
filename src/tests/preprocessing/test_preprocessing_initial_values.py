...
def sample_multiple_initial_values(vec_0, n_points, remove_trend, normalization, n_repl, obs_noise, var, diff_lengths=False):

    list_x, list_t, trends, means, std_devs = [], [], [], [], []

    if diff_lengths:
        for i in range(n_repl/2): #TODO: change
        ...
        for j in range(n_repl - n_repl/2 - 1):
            x_0 = np.random.normal(vec_0[0], var)
            y_0 = np.random.normal(vec_0[1], var)
            z_0 = np.random.normal(vec_0[2], var)
...
def modelling(vec_0, train_length, remove_trend, normalization, n_repl, obs_noise, var):
    ...
    xs, ts, trends, means, std_devs = sample_multiple_initial_values(vec_0, train_length+25, remove_trend, normalization, n_repl, obs_noise, var, diff_lengths=False)
